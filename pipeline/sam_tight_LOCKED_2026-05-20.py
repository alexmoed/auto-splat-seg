    ...
    for cam in cam_data["cameras"]:
        ...
        if parent_label:
            ...
            if bbox_norm is None:
                # Qwen-not-found: use the FULL frame (no crop) instead of
                # skipping the view. SAM still tries to segment the parent
                # label; bad masks get outvoted. Removed 2026-05-20 — was
                # another skip-pattern band-aid (low-pitch views of
                # surrounded objects like the armchair often fail
                # qwen_view_bbox but the SAM masks are still useful).
