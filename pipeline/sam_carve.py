...
SAM_PAD_M = 0.2                  # mask dilation in meters (was 0.5 — too wide,
                                  # 97% retention = no carving). 0.2 gives
                                  # ~130px dilation at 1.2m object distance.
...
def step2_derive_sam_prompt(scene_dir: Path, obj_dir: Path):
    ...
    if not images:
        ...
        # input pngs we have (the steeper p-45 views still see the object).
        for p in sorted(diag.glob("input_*_p-15.png")) + \
                 sorted(diag.glob("input_*_p-45.png")):
            tag = p.stem.replace("input_", "")
            images.append((tag, p))
            if len(images) >= 4:
                break
    if not images:
        sys.exit(f"[fatal] no input_*.png views in {diag}\n  run --step 1 first")
